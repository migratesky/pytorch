"""
Tests for lazy tensor operations with meta reference disabled.

This test suite verifies that lazy tensor operations work correctly when the functionalization
meta reference is disabled. This is important for ensuring that PyTorch's lazy tensor
system works properly in different configurations.

Note: This is a migration of the original test_ts_opinfo.py to pytest.
"""
import functools
import itertools
import os
...
# Initialize lazy backend
torch._lazy.ts_backend.init()


def get_test_device():
    """Get the test device (cuda if LTC_TS_CUDA is set, cpu otherwise)."""
    return "cuda" if "LTC_TS_CUDA" in os.environ else "cpu"


def remove_suffixes(l):
    """Remove suffixes from a list of strings."""
    return [x.split(".")[0] for x in l]


def init_lists():
    """Initialize lists of ops and configurations from yaml file."""
    # Get the PyTorch root directory
    pytorch_root = Path(os.path.abspath(os.path.dirname(__file__))).parent
    TS_NATIVE_FUNCTIONS_PATH = pytorch_root / "aten/src/ATen/native/ts_native_functions.yaml"
...
def clone_move(t):
    """Clone a tensor and move it to lazy device."""
    dev = "lazy"
...
class TestLazyTensor(JitTestCase):
    """Test suite for basic lazy tensor operations."""

    @pytest.fixture(autouse=True)
    def setup_env(self):
        """Setup and teardown for the environment variable."""
        original_value = os.environ.get('TORCH_DISABLE_FUNCTIONALIZATION_META_REFERENCE')
        os.environ['TORCH_DISABLE_FUNCTIONALIZATION_META_REFERENCE'] = '1'
        yield
        if original_value is not None:
            os.environ['TORCH_DISABLE_FUNCTIONALIZATION_META_REFERENCE'] = original_value
        else:
            del os.environ['TORCH_DISABLE_FUNCTIONALIZATION_META_REFERENCE']

    @pytest.mark.skip("Disable until autograd supports symints")
    ...
    def test_view_mark_step_preserved(self):
        """Test that view operations preserve mark_step behavior."""
        test_device = get_test_device()
    ...
    def test_tensor_ctr(self):
        """Test tensor constructor with lazy device."""
        test_device = get_test_device()
...
class TestLazyOpInfo(TestCase):
    """Test suite for lazy tensor operator information."""
    ...
    def test_dispatched_to_lazy(self, device, dtype, op):
        """Test that ops are correctly dispatched to lazy backend."""
    ...
    def test_correctness(self, device, dtype, op):
        """Test numerical correctness of lazy tensor operations."""
        test_device = get_test_device()
        ...
        def assert_allclose_rec(t):
            ...
            if isinstance(a, torch.Tensor):
                assert torch.allclose(clone_to_device(a, test_device), b, atol=1e-4)
...
class TestLazyDynamicOps(TestCase):
    """Test suite for lazy tensor operations with dynamic shapes."""

    @pytest.fixture(autouse=True)
    def setup_dynamic_mode(self):
        """Setup and teardown for dynamic shape mode."""
        old_mode = torch._C._lazy._get_symbolic_shape_mode()
        torch._C._lazy._set_symbolic_shape_mode(True)
        yield
        torch._C._lazy._set_symbolic_shape_mode(old_mode)

    def test_nonzero_dynamic(self):
        """Test nonzero operation with dynamic shapes."""
        test_device = get_test_device()
        x1 = torch.tensor(
        ...
        assert tuple(x2_eager.size()) == (3, 2)

    def test_adaptiveavgpool3d_dynamic(self):
        """Test adaptive average pooling with dynamic shapes."""
        ...
        assert out_cpu.shape == out_lazy.shape
